...
int main(int argc, char * argv[])
{
	if (argc == 2)
	{
		...
		std::vector<file>* fileNames = type.filesReturn(param);
		...
		std::cout << " PARALLE TYPE ANALYSIS RESULT (MERGED TYPE TABLE)" << std::endl;
		std::cout << "----------------------------------------------------------------------------" << std::endl;
		typetable->show();
		for (size_t i = 0; i < fileNames->size(); i++)
		...
		std::cout << "PARALLEL DEPENDENCY ANALYSIS RESULT" << std::endl;
		std::cout << "---------------------------------------------------------------------" << std::endl;
		while (it != fin.end())
		{
